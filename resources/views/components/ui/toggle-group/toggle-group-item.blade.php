...
<button
	...
	aria-pressed="false"
	x-bind:aria-pressed="selected === {{ Illuminate\Support\Js::from($value) }}"
	x-bind:data-state="selected === {{ Illuminate\Support\Js::from($value) }} ? 'on' : 'off'"
	x-on:click="selected = {{ Illuminate\Support\Js::from($value) }}; $dispatch('{{ $changeEvent }}', { value: {{ Illuminate\Support\Js::from($value) }} })"
>
	{{ $slot }}
</button>
